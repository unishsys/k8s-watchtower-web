    ...
    <q-page flex flex-center class="q-ma-lg">
      ...
      <div class="row no-wrap justify-around">
        ...
        <apexchart
          type="pie"
          width="380"
          :options="piChartOptions.chartOptions"
          :series="piChartOptions.series"
          class="overflow-auto"
        >
        </apexchart>
      </div>
          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              ...
              <q-td
                key="name"
                :props="props"
                @click.stop="getPodLogs(props.row)"
              >
...
function getNodeInfo() {
  api.get(`/api/v1/nodes/`).then((response) => {
    nodeTable.value = response.data.data;
    barChartOptions.value.series[0].data = response.data.data.map((item) => ({
      x: item.nodeName,
      y: item.containerCount,
    }));
    barChartOptions.value.series[1].data = response.data.data.map((item) => ({
      x: item.nodeName,
      y: parseInt(item.cpuUsage / (1024 * 1024)),
    }));
    ...
    piChartOptions.value.series = response.data.data.map(
      (item) => item.memoryUsage / (1024 * 1024),
    );
    piChartOptions.value.chartOptions.labels = response.data.data.map(
      (item) => item.nodeName,
    );
  });
}
...
function getPodInfo() {
  api.get(`/api/v1/pods/`).then((response) => {
...
function getPodLogs(rowData) {
  ...
  let uri = `ws://localhost:8081/api/v1/logs/pod/${rowData.namespace}/${rowData.name}`;
